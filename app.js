...
function render(){
  ...
  if(tab==='home'){
    ...
    c.innerHTML+=`<div class='card'><h2>Join Competition</h2><p><b>New player?</b> Register your name and create a 4-digit PIN.</p><button class='primary' id='joinBtn'>Join Competition</button><p class='muted'><b>Already joined?</b> Tap Make Pick in the menu to choose your team.</p></div>`;
    $('#joinBtn').onclick=()=>{const name=prompt('Enter your name:');if(!name||!name.trim())return;const pin=prompt('Create a 4-digit PIN:');if(!/^\d{4}$/.test(pin||''))return notice('PIN must be exactly 4 digits.','warn');if(state.players.some(p=>p.name.toLowerCase()===name.trim().toLowerCase()))return notice('That player already exists.','warn');state.players.push({name:name.trim(),pin,alive:true,picks:{},used:[],eliminatedRound:null});notice(`${name.trim()} joined the competition.`);render()};
  }
  if(tab==='pick'){
   ...
   $('#playerSel').onchange=e=>{const target=state.players.find(p=>p.name===e.target.value);if(target&&target.pin){const pin=prompt(`Enter PIN for ${target.name}:`);if(pin!==target.pin){notice('Incorrect PIN.','warn');render();return}}state.selectedPlayer=e.target.value;notice('');render()};
